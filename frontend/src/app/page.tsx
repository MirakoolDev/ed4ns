...
import { FACTORY_ADDRESS_BASE, FACTORY_ADDRESS_V2_BASE, FACTORY_ADDRESS_ROBINHOOD, STANDALONE_GAMES, getExplorerUrl, artworkImageSrc } from "@/config";
...
export function GameCard({ address, version, targetChainId }: { address: string; version: string; targetChainId: number }) {
  ...
  const artworkUrl = artworkURI ? artworkImageSrc(artworkURI) : "";
      ...
      <div style={{ padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 2 }}>
          {[
            ...
            { label: "Pool", value: pool !== undefined ? `${parseFloat(Number(formatEther(pool)).toFixed(4))}` : "—" },
            { label: "Price", value: price !== undefined ? `${parseFloat(Number(formatEther(price)).toFixed(4))}` : "—" },
...
export default function HomeGallery() {
  ...
  useEffect(() => {
    fetch("/api/admin/hidden-games")
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setHiddenGames(data);
      })
      .catch(console.error);
  }, []);
  ...
  ].filter(game => !hiddenGames.includes(game.address));
        ...
        <div className="hero-inner">
          ...
          <div className="hero-steps">
            ...
            <div className="hero-step">
              ...
              <div className="hero-step-desc">Each round, ~half the tokens are eliminated using on-chain randomness powered by Dice Protocol.</div>
            </div>
            <div className="hero-step-divider" />
            <div className="hero-step">
